...
Capture a real, non-blank screenshot of a web-based game or app without a display, for visual critique loops (visual-loop / visual-reference-loop capture steps), visual QA, or regression evidence. Verified with headless Chromium via playwright-core + the system executable.
...
3. **Reach meaningful state, not a menu.** Prefer the app's own deterministic boot hooks (e.g. a `?smoke=1` URL param that boots to a known state). If none exist, click through the real UI via playwright selectors.
...
- Headless chromium may lack the app's display assumptions — keep capture args minimal (`--no-sandbox --disable-gpu`).
...
`references/target-capture-recipe.md` — a fill-in template recipe (boot hook, state check, exact env-var command, artifact conventions).
